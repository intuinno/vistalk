            ...
            selectedEntity.each(function (d, i) {
                ...
                selectionCircle.attr("cx",function (d) {
                    ...
                    return (xSelect(d))//+transSx);
                        ...
                        return rSelect(+d.numReview) * 1;//factor;
                    }).attr("fill", color).attr("stroke", color).classed("selectedCircle", true)
                    .attr("opacity", "0.5")
                    .attr("stroke-width", String(5 * factor))
                ...
                selectionCircle.attr("cx",function (d) {
                    ...
                    //return xQuery(d);
                        ...
                        if (scalQxy > 20) {
                            ...
                            var factor = 1 / scalQxy;
                        } else {
                            var factor = 1;
                        }
                        return rQuery(+d.numReview) * 1//factor;
                    }).attr("fill", color).attr("stroke", color).classed("selectedCircle", true)
                    .attr("opacity", "0.5");
//                    .attr("stroke-width", "5")
//                    .attr("stroke-opacity", "0.5")
                ...
                selectionCircle.attr("cx",function (d) {
                    ...
                    return (xSelect(d))//+transSx);
                        ...
                        if (scalSxy > 20) {
                            ...
                            var factor = 1 / scalSxy;
                        } else {
                            var factor = 1;
                        }
                        return rSelect(+d.numReview) * factor;
                    }).attr("fill", color).attr("stroke", color).classed("selectedCircle", true)
                    .attr("opacity", "0.5");
//                    .attr("stroke-width", "5")
//                    .attr("stroke-opacity", "0.5")
    ...
    d3.csv(STATIC_URL + "yelpvis/data/business_word.csv", function (ratingsCSV) {
    ...
    d3.json(STATIC_URL + "yelpvis/data/us-states.geojson", function (states) {

        myStates = states;

    })

    d3.csv(STATIC_URL + "yelpvis/data/word.csv", function (movieCSV) {
...
//	PanZoomTool.nozoomUser = d3.behavior.zoom().x(xScaleUser).y(yScaleUser).on("zoom", null);
    ...
    d3.csv(STATIC_URL + "yelpvis/data/business.csv", function (userCSV) {
    ...
    function zoomedMovie() {
        ...
        //svgMovieGroup.selectAll(".movieSVGGroup .star")[0].attr("r");
    ...
    VisDock.SaveAsJson = function () {
        ...
        if (isMovieSelected) {
            ...
            for (i = 0; i < selectionStatesMovie.querySetsList.length; i++) {
                ...
                json_class.contourList.push(selectionStatesMovie.querySetsList[i].contourList)
                json_class.mode.push(selectionStatesMovie.querySetsList[i].mode)
                json_class.relationMin.push(selectionStatesMovie.querySetsList[i].relationMin)
                json_class.relationMax.push(selectionStatesMovie.querySetsList[i].relationMax)
                json_class.contourMode.push(selectionStatesMovie.querySetsList[i].contourMode)
                json_class.contourOn.push(selectionStatesMovie.querySetsList[i].contourOn)
            }

            //  json_class.contourList = selectionStatesMovie.contourList
        } else {
...
//        }
        ...
        $.ajax({
            ...
            success: function (result) {
                console.log(result);
                alert("The comment has been submitted to Server.")
            }
        });
